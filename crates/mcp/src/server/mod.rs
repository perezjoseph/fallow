...
impl FallowMcp {
    ...
    #[tool(
        description = "Returns unverified security candidates, not confirmed vulnerabilities. Runs `fallow security --format json` and returns `kind: \"security\"`, `security_findings`, category, CWE, severity, evidence, structural trace, reachability ranking context, and blind-spot counters for agent verification. `severity` is a review-priority tier, not a verified vulnerability verdict. Each finding also carries an agent-actionable `candidate` record (`source_kind`, the untrusted-input kind as a stable catalogue id or null; a self-contained `sink` with the captured callee; and the `boundary` crossed), an optional `taint_flow` source-to-sink triple (present only when an untrusted source is import-reachable to the sink), and a stable `finding_id` (equal to the SARIF fingerprint) for correlating a candidate across runs. Set `surface: true` to include the top-level `attack_surface` inventory with source-to-sink paths and defensive-boundary verification prompts. There is no `impact` field: deciding exploitability is the agent's job. `reachability.untrusted_source_trace` is module-level context only and does not prove value flow. `reachability.taint_confidence` tiers reachable candidates as `arg-level` or `module-level`; tier from that field rather than evidence prose. Verify trace, reachability context, severity, and evidence before editing code or presenting a finding as a vulnerability. Supports root, config, workspace, changed_since, paths, changed_workspaces, surface, no_cache, and threads. Use paths for the agent edit loop: it forwards to `fallow security --file` and scopes returned candidates to matching finding anchors, trace hops, or untrusted-source reachability trace hops while cached analysis keeps reruns fast. The CLI also honors `FALLOW_DIFF_FILE` from the MCP server environment for line-level diff scoping. Security analysis can exceed the default 120s subprocess timeout on large repos; raise `FALLOW_TIMEOUT_SECS` accordingly.",
        annotations(read_only_hint = true, open_world_hint = true)
    )]
